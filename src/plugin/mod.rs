...
pub fn check_for_collisions(mut query: Query<(&mut Transform, &mut Collider, Option<&RigidBody>)>) {
    let mut iter = query.iter_combinations_mut();
    while let Some([(mut trafo1, mut collider1, body1), (mut trafo2, mut collider2, body2)]) =
        iter.fetch_next()
    {
        if let Some(collision) = match &collider1.shape {
            Shape::Circle(circle) => match &collider2.shape {
                Shape::Circle(other) => circle.collides(other, &trafo1, &trafo2),
                Shape::ConvexPolygon(other) => circle.collides(other, &trafo1, &trafo2),
            },
            Shape::ConvexPolygon(polygon) => match &collider2.shape {
                Shape::Circle(other) => polygon.collides(other, &trafo1, &trafo2),
                Shape::ConvexPolygon(other) => polygon.collides(other, &trafo1, &trafo2),
            },
        } {
